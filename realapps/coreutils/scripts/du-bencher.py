...
def compose_du_cmd_env(libforeactor, workdir, use_foreactor, backend,
                       pre_issue_depth):
...
def run_du_single(libforeactor, workdir, use_foreactor, backend=None,
    ...
    cmd, env = compose_du_cmd_env(libforeactor, workdir, use_foreactor, backend,
                                  pre_issue_depth)
...
def gen_du_util(dev_name, libforeactor, workdir, use_foreactor, backend=None,
                pre_issue_depth=0):
    os.system("ulimit -n 65536")
    os.system("sudo sync; sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'")
...
def run_utils(libforeactor, workdir, output_log, backend, pre_issue_depth_list,
    ...
    with open(output_log, 'w') as fout:
        cpu_util, disk_r_util, disk_w_util = \
            gen_du_util(dev_name, libforeactor, workdir, False)
        ...
        for pre_issue_depth in pre_issue_depth_list:
            cpu_util, disk_r_util, disk_w_util = \
                gen_du_util(dev_name, libforeactor, workdir, True, backend,
                            pre_issue_depth)
