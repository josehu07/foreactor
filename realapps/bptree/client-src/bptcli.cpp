...
static void fuzzy_test_round(const std::string& filename,
                             size_t num_to_load) {
    ...
    bptree::BPTree<uint64_t, uint64_t> bptree(filename, TEST_DEGREE);
...
static std::tuple<std::vector<bptree_req_t>, size_t> read_input_trace(
    ...
    if (degree < 2 || degree > bptree::MAXNKEYS) {
...
static std::tuple<unsigned, unsigned> execute_input_trace(
        bptree::BPTree<uint64_t, uint64_t>& bptree,
        const std::vector<bptree_req_t>& reqs,
        uint64_t value, std::vector<double>& microsecs) {
...
int main(int argc, char *argv[]) {
    ...
    cmd_args.add_options()
        ...
        ("w,workload", "workload trace", cxxopts::value<std::string>(trace)->default_value(""));
    ...
    bptree::BPTree<uint64_t, uint64_t> bptree(file, degree);
