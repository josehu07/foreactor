...
def run_exprs(libforeactor, dbdir, trace, drop_caches, output_prefix,
              pre_issue_depth_list):
    output = run_ycsbcli_single(libforeactor, dbdir, trace, drop_caches, False)
    with open(output_filename(output_prefix, 'orig'), 'w') as fout:
        fout.write(output)

    for pre_issue_depth in pre_issue_depth_list:
        output = run_ycsbcli_single(libforeactor, dbdir, trace, drop_caches,
                                    True, pre_issue_depth)
...
def main():
    ...
    run_exprs(args.libforeactor, args.dbdir, args.trace, args.drop_caches,
              args.output_prefix, args.pre_issue_depths)
